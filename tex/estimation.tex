...
在完成信号检测之后，雷达系统还需要进一步提取目标的关键参数，例如方位角、距离、多普勒频率以及极化特征等。这些参数的准确估计不仅关系到目标的定位与跟踪精度，也是后续态势感知与目标识别的重要基础。从算法发展的角度来看，目标参数估计大体经历了几个阶段：从最初的经典波束形成方法，到最小方差处理方法，再到子空间方法和稀疏表示方法。近年来，随着多维观测手段的兴起，多域联合估计方法逐渐受到关注。这类方法能够同时利用空间、多普勒以及极化等多维信息，从而实现更强的参数解耦和更高的估计精度。

在上一章中，我们介绍了雷达的接收信号模型。对于一个阵列多普勒雷达系统，其接收信号可以表示为一个张量：
\[
    \begin{split}
...
\]
其中\( \mathbf{R} = \begin{bsmallmatrix} \bm{r}_1 & \bm{r}_2 & \cdots & \bm{r}_K \end{bsmallmatrix} \in \mathbb{C}^{L \times K} \)为目标信号矩阵，\( \mathbf{A} = \begin{bsmallmatrix} \bm{a}_{\omega_1} & \bm{a}_{\omega_2} & \cdots & \bm{a}_{\omega_K} \end{bsmallmatrix} \in \mathbb{C}^{M \times K} \)为阵列流形矩阵（导向矩阵），其列向量为目标的空间或多普勒流形向量，其第\( k \)列通常有如下形式：
\[
    \bm{a}_{\omega_k} = \begin{bmatrix}
        1 & e^{j 2 \pi \omega_k} & \cdots & e^{j 2 \pi (M-1) \omega_k }
    \end{bmatrix}^{\mathrm{T}}.
\]
其中，参数 $\omega_k$ 的物理意义取决于雷达体制：若为均匀线性阵列雷达，则 $\omega_k = \frac{d}{\lambda}\sin \theta_k$，即与目标的方位角 $\theta_k$ 相关；若为单阵元多脉冲雷达，则 $\omega_k = \frac{2 T_r v_k}{\lambda}$，即与目标的径向速度 $v_k$ 相关。

波束形成（Beamforming）是最经典的参数估计方法之一，其基本思想是通过对接收信号进行加权求和，形成一个指向特定方向的空间滤波器或对应特定速度的多普勒滤波器，从而增强该方向或速度上的信号分量。对于给定的滤波器系数\( \bm{h}_{\omega} \)，波束形成的输出可以表示为：
\[
    \bm{y}_{\omega} = \mathbf{X} \bm{h}_{\omega} \in \mathbb{C}^{L \times 1}.
\]
该输出可以看作是某个特定方向或速度上的回波响应。
...
\[
    \bm{h}_{\omega} = \frac{1}{M} \overline{\bm{a}}_\omega
    = \frac{1}{M} \begin{bmatrix}
        1 & e^{-j 2 \pi \omega} & \cdots & e^{-j 2 \pi (M-1) \omega}
    \end{bmatrix}^{\mathrm{H}},
\]
即可有效提取出对应参数值 $\omega$ 的信号分量。这种将滤波器权向量选为与目标流形向量相匹配的策略，又称为 \textbf{Bartlett 波束形成}，是最经典的一种波束形成策略。

进一步地，通过扫描不同的参数值\( \omega \)，将原始信号投影到不同的方向或速度上，则可以分别获得不同方向或速度上的信号分量。具体而言，对于阵列信号，可以构建如下矩阵：
\[
    \mathbf{H} = \frac{1}{M}\begin{bmatrix}
        1                                                   & 1                                                    & \cdots & 1                                                   \\
...
\[
    ...
    \end{bmatrix} \in \mathbb{C}^{M \times N},
\]
其中，\( v_1, v_2, \cdots, v_N \)为扫描的速度值。对应的波束形成可表示为以下矩阵乘法：
\[
    \mathbf{Y} = \mathbf{X} \mathbf{H} \in \mathbb{C}^{L \times N}.
\]
值得注意的是，对于多脉冲信号，当所选扫描速度 $v_n$ 满足如下条件时：
\[
    \frac{2 T_r v_n}{\lambda} = \frac{n-1}{M}, \quad n = 1, 2, \cdots, N.
\]
波束形成矩阵 $\mathbf{H}$ 正好对应于一个 $M \times M$ 的离散傅里叶变换（Discrete Fourier Transform，DFT）矩阵的前 $N$ 行。特别地，当 $M = N$ 时，$\mathbf{H}$ 即为标准的 DFT 矩阵。在这种情况下，波束形成过程等价于对接收信号在参数维度（慢时间维）上进行离散傅里叶变换。
...
\[
    \mathbf{Z} = \mathbf{S}^{\mathrm{H}} \mathbf{X}\mathbf{H}.
\]
...
\begin{solution}
    处理结果如\cref{fig_compressed}所示。\cref{fig_compressed_1} 给出了对 \( \mathbf{X} \) 的列向量进行脉冲压缩的结果：原本具有一定展宽的目标回波在压缩后转变为尖锐的脉冲，在图像中表现为水平直线；同时，由于两个目标对应的直线具有不同的频率起伏，说明其参数值（方位或速度）存在差异。

    \cref{fig_compressed_2} 展示了对 \( \mathbf{X} \) 的行向量进行波束形成的结果：两个目标在参数维上也被压缩为尖锐的脉冲，在图像中对应两条垂直直线，并且这两条直线明显呈现出 Chirp 信号的特征。

    \cref{fig_compressed_3} 则给出了在列向量和行向量上分别进行脉冲压缩与波束形成的联合结果：此时，两个目标在距离维和参数维均被压缩为尖锐脉冲，在图像中表现为两个亮点，其位置正好对应目标的距离和参数值。
    ...
    \begin{figure}[htb!]
        ...
        \begin{subfigure}{.3\textwidth}
            ...
            \begin{tikzpicture}
                ...
                \end{axis}
            \end{tikzpicture}
            \caption{\( \mathbf{S}^{\mathrm{H}}\mathbf{X} \)实部}
        ...
        \begin{subfigure}{.3\textwidth}
            ...
            \begin{tikzpicture}
                ...
                \end{axis}
            \end{tikzpicture}
            \caption{\(  \mathbf{S}^{\mathrm{H}}\mathbf{X}\mathbf{H} \)幅度}
...
在某些情况下，我们可能并不关心目标的距离信息，而仅希望获得其参数信息（如方位或速度）。此时，可以直接利用波束形成后的输出，来计算参数维度上的功率谱。具体地，对于参数值 $\omega$，对应输出的功率可表示为:
\[
    p_{\omega} = \frac{1}{L}\|\bm{y}_{\omega}\|_2^2
    = \frac{1}{L}\|\mathbf{X}\bm{h}_{\omega}\|_2^2
    = \frac{1}{L}\bm{h}_{\omega}^{\mathrm{H}}\mathbf{X}^{\mathrm{H}}\mathbf{X}\bm{h}_{\omega}
    = \bm{h}_{\omega}^{\mathrm{H}} \mathbf{\Sigma}_{\mathbf{X}} \bm{h}_{\omega},
\]
...
\[
    \bm{y}_{\omega} = \mathbf{X} \bm{h}_{\omega} = \sum_{k=1}^{K} \bm{r}_k \bm{a}_{\omega_k}^{\mathrm{T}} \bm{h}_{\omega} = \sum_{k=1}^{K} \bm{r}_k \frac{\bm{a}_{\omega_k}^{\mathrm{T}} \overline{\bm{a}}_{\omega}}{M}.
\]
从中可以发现，波束形成的输出实际上是各个目标回波信号的加权和，对应的权重为\( \frac{\bm{a}_{\omega_k}^{\mathrm{T}} \overline{\bm{a}}_{\omega}}{M} \)。理想情况下，当目标参数值与滤波器参数值相匹配时，该权重接近1，否则趋于0。但在实际应用中，\( M \)的数值有限。此时，即便\( \omega_k \neq \omega \)，对应的权重也可能较大，导致提取出来的信号中混入了其他目标的成分，从而影响估计精度。
...
\[
    ...
    \end{split}
\]
可以发现该权重的取值仅和\( M \)以及参数差值\( \omega_k - \omega \)有关。如\cref{fig_weight}所示，随着\( M \)的增大，权重函数的主瓣变窄，旁瓣降低，对应的分辨率也随之提升。
...
\subsection{和差比幅测角}
无源相控阵虽然节约了硬件开销，但无法同时观测多个方向的信号分量；有源相控阵虽然提升了灵活性，却使硬件成本大大增加。为此，雷达系统中常采用一种折中的方案，即和差比幅测角（Sum–Difference Amplitude Comparison）。 该方法通过将阵列划分为多个子阵，并对各子阵的输出进行加权组合，从而在一定程度上兼顾了硬件复杂度与参数估计性能。

不妨考虑一个均匀线性阵列，其阵元总数为 $2M$，并将其划分为两个子阵，每个子阵包含 $M$ 个阵元。对于给定方位角 $\theta$，和差比幅测角方法构造如下两组滤波器：
\[
    \bm{h}_{\text{sum}} = \frac{1}{2M}
    \begin{bmatrix}
        \bm{h}_1 \\
        \bm{h}_2
    \end{bmatrix}, \quad  \bm{h}_{\text{diff}} = \frac{1}{2M}
    \begin{bmatrix}
        \bm{h}_1 \\
        -\bm{h}_2
    \end{bmatrix},
\]
其中，\( \bm{h}_1 \) 和 \( \bm{h}_2 \) 分别为前\( M \) 个阵元和后\( M \) 个阵元的权向量，有如下形式：
\[
    \bm{h}_1 = \begin{bmatrix}
        1                                           \\
        e^{- j 2 \pi \frac{d \sin \theta}{\lambda}} \\
        \vdots                                      \\
        e^{- j 2 \pi \frac{(M-1) d \sin \theta}{\lambda}}
    \end{bmatrix}, \quad
    \bm{h}_2 = \begin{bmatrix}
    ...
    \end{bmatrix}.
\]
记 $\bm{h}_{\text{sum}}$ 为两个子阵的和波束，$\bm{h}_{\text{diff}}$ 为两个子阵的差波束。对于位于方位角 $\theta_k$ 的目标，其波束形成输出分别为
\[
    \bm{y}_{\text{sum}} =  \bm{r}_k \bm{a}_{\omega_k}^{\mathrm{T}} \bm{h}_{\text{sum}},
    \quad
    \bm{y}_{\text{diff}} = \bm{r}_k \bm{a}_{\omega_k}^{\mathrm{T}} \bm{h}_{\text{diff}}.
\]
可以看出，两者在时域波形上保持完全一致，仅在复数比例因子上有所不同，分别为$\bm{a}_{\omega_k}^{\mathrm{T}} \bm{h}_{\text{sum}}$ 和 $\bm{a}_{\omega_k}^{\mathrm{T}} \bm{h}_{\text{diff}}$。该比例因子同时包含幅度与相位信息，其比值正是和差比幅测角所利用的参数。
...
\[
    \begin{split}
        \bm{a}_{\omega_k}^{\mathrm{T}} \bm{h}_{\text{sum}} & = \frac{1}{2M}\begin{bmatrix}
        ...
        \begin{bmatrix}
            ...
            \bm{h}_2
        \end{bmatrix}                                                                                                                                                                                                                                                                                  \\
                                                           & = \frac{1}{2M} \bm{a}_{\omega_k,1}^{\mathrm{T}} \bm{h}_1 + \frac{1}{2M} \bm{a}_{\omega_k,2}^{\mathrm{T}} \bm{h}_2                                                                                                                          \\
                                                           & = \frac{1}{2M} \bm{a}_{\omega_k,1}^{\mathrm{T}} \bm{h}_1 + \frac{1}{2M} \left( e^{j 2 \pi \frac{M d \sin \theta_k}{\lambda}} \bm{a}_{\omega_k,1}^{\mathrm{T}} \right) \left( e^{-j 2 \pi \frac{M d \sin \theta}{\lambda}} \bm{h}_1 \right) \\
                                                           & = \frac{1 + e^{j 2\pi \frac{M d (\sin \theta_k - \sin \theta)}{\lambda}}}{2M} \bm{a}_{\omega_k,1}^{\mathrm{T}} \bm{h}_1.
    \end{split}
\]
类似地，对于差波束，有
\[
    \begin{split}
        \bm{a}_{\omega_k}^{\mathrm{T}} \bm{h}_{\text{diff}} & = \frac{1}{2M}\begin{bmatrix}
                                                                                \bm{a}_{\omega_k,1}^{\mathrm{T}} & \bm{a}_{\omega_k,2}^{\mathrm{T}}
                                                                            \end{bmatrix}
        \begin{bmatrix}
            \bm{h}_1 \\
...
\[
    \sin \theta_k - \sin \theta = 2 \cos \left( \frac{\theta_k + \theta}{2} \right) \sin \left( \frac{\theta_k - \theta}{2} \right) \approx 2 \cos \theta  \left( \frac{\theta_k - \theta}{2} \right) = (\theta_k - \theta) \cos \theta .
\]
将其代入上式，得到
\[
...
\cref{fig_ac}展示了\( \theta \)等于0时，目标角度\( \theta_k \)与和差比幅值\( r \)之间的关系曲线。可以看到，随着目标角度偏离扫描角度，和差比幅值呈现出非线性变化的趋势。通过测量和差比幅值\( r \)，并结合上述关系式，即可实现对目标方位角\( \theta_k \)的估计。
...
经典波束形成方法并未考虑噪声与干扰的统计特性，因此在复杂环境下性能往往受到限制。为此，最小方差无失真响应（Minimum Variance Distortionless Response, MVDR）方法在设计滤波器时引入了噪声与干扰的协方差矩阵，通过优化权向量以最小化输出功率，同时保证对目标信号的无失真响应。该方法的发明者为 Jack Capon，因此有时也称为 Capon 算法。

设 \( \bm{h}_{\omega} \) 为待设计的滤波器权向量，则对应的输出为
\[
    \bm{y}_{\omega} = \mathbf{X} \bm{h}_{\omega}.
\]
直观上，我们希望输出 \(\bm{y}_{\omega}\) 尽可能只包含参数值为 \(\omega\) 的目标信号，同时抑制其他信号与噪声干扰。注意到目标信号对应的导向向量为 \(\bm{a}_{\omega}\)，因此目标分量在输出中的占比与 \(|\bm{a}_{\omega}^{\mathrm{T}} \bm{h}_{\omega}|\) 成正比。由此可构造如下优化问题：
\[
    \max_{\bm{h}_{\omega}} \frac{|\bm{a}_{\omega}^{\mathrm{T}} \bm{h}_{\omega}|^2}{ \frac{1}{L} \|\bm{y}_{\omega}\|_2^2},
\]
即在保证输出功率尽可能小的前提下，使目标信号成分最大化。该问题可转化为如下形式的约束优化问题：
\[
    \begin{cases}
        \min_{\bm{h}_{\omega}} \quad & \frac{1}{L} \|\bm{y}_{\omega}\|_2^2               \\
        \text{s.t.} \quad            & \bm{a}_{\omega}^{\mathrm{H}} \bm{h}_{\omega} = 1,
    \end{cases}
\]
...
\[
    \mathbf{H} = \begin{bmatrix} \bm{h}_{\omega_1} & \bm{h}_{\omega_2} & \cdots & \bm{h}_{\omega_N} \end{bmatrix},
\]
并由此得到波束形成的输出矩阵
\[
...
\end{example}
\begin{solution}
    二维脉压结果如\cref{fig_compressed_capon}所示。与经典波束形成相比，Capon 方法在参数维度上展现出更高的分辨率，两个目标在参数维上被压缩为更尖锐的脉冲，并且旁瓣水平显著降低，从而提升了目标的可分辨性和检测性能。
    \begin{figure}[htb!]
        \centering
    ...
    参数维度的功率谱估计如\cref{fig_compressed_capon2}所示。可以看到，Capon 方法在参数维度上展现出更高的分辨率，两个目标被压缩为更尖锐的脉冲，并且旁瓣水平显著降低，有效提升了目标的可分辨性和检测性能。
    \begin{figure}[htb!]
        \centering
        \begin{tikzpicture}
            \begin{axis}[
...
在实际应用中，由于接收信号的样本数量有限，协方差矩阵的估计往往不够准确，甚至可能不可逆。为解决这一问题，可以借鉴岭回归（Ridge Regression）的思想，在目标函数中加入权重向量的范数惩罚项，从而得到如下优化模型：
\[
    \begin{cases}
        \min\limits_{\bm{h}_{\omega}} \quad &
        \bm{h}_{\omega}^{\mathrm{H}} \mathbf{\Sigma}_{\mathbf{X}} \bm{h}_{\omega}
        + \delta \|\bm{h}_{\omega}\|_2^2                                                        \\
        \text{s.t.} \quad                   & \bm{a}_{\omega}^{\mathrm{T}} \bm{h}_{\omega} = 1,
    \end{cases}
\]
...
\section{子空间方法}
从滤波的角度来看，经典波束形成与 Capon 算法在本质上均通过设计权向量，对接收信号矩阵进行加权叠加，以增强特定参数对应的目标回波并抑制噪声与干扰。与经典波束形成相比，Capon 算法利用接收数据的协方差矩阵，在无失真响应约束下最小化输出功率，因此在空间干扰抑制方面具备更优性能。然而，该类方法仍然受限于``滤波—加权叠加''的处理框架：在纯白噪声条件下，由于噪声在空间上各向同性，其输出噪声功率难以进一步降低，从而形成了分辨力与抑制性能的理论瓶颈。

为突破这一局限，研究者提出了基于子空间的高分辨率方法。该类方法不再将权向量设计作为核心手段，而是对接收信号的协方差矩阵进行特征分解，将信号与噪声划分到互相正交的子空间，并利用它们之间的几何结构关系实现参数估计。得益于这一子空间建模思想，该方法能够获得远超传统滤波框架的分辨能力。典型代表包括 MUSIC 与 ESPRIT 算法，它们均基于信号子空间与噪声子空间的正交特性，实现了高精度、高分辨率的参数估计。
...
对于信号部分，一般假设各目标对应的信号互不相关，即 \(\mathbf{R}\) 的列向量互相正交。此时信号协方差矩阵可写为
\[
    ...
    \mathbf{A}^{\mathrm{T}}
    = \overline{\mathbf{A}}\,\mathbf{\Sigma}_{\mathbf{R}}\,\mathbf{A}^{\mathrm{T}},
\]
其中
\[
    \mathbf{\Sigma}_{\mathbf{R}}
    = \frac{1}{L}\mathbf{R}^{\mathrm{H}}\mathbf{R}
    \approx \operatorname{diag}(\sigma_1^2,\sigma_2^2,\cdots,\sigma_K^2),
\]
...
\[
    \begin{split}
        ...
        \big(\mathbf{R}\mathbf{A}^{\mathrm{T}}+\mathbf{N}\big)                                         \\
         & \approx \frac{1}{L}\mathbf{\bar{A}}\mathbf{R}^{\mathrm{H}}\mathbf{R}\mathbf{A}^{\mathrm{T}}
        + \frac{1}{L}\mathbf{N}^{\mathrm{H}}\mathbf{N}                                                 \\
...
考虑到矩阵 \(\mathbf{A}\) 的列向量由不同频率的复指数信号组成，而当阵元数 \(M\) 远大于目标数 \(K\) 时，不同频率的复指数信号近似正交。因此，对于协方差矩阵 \(\mathbf{\Sigma}_{\mathbf{X}}\)，不难验证 \(\mathbf{A}\) 的列向量的共轭 \(\overline{\bm{a}}_{\omega_k}\) 是其特征向量，对应的特征值为 \(\sigma_k^2 M + \sigma^2\)。具体推导如下：
\[
    \begin{split}
...
另一方面，一般有 \(M > K\)，因此 \(\mathbf{\Sigma}_{\mathbf{X}}\) 至少还存在 \(M-K\) 个不在 \(\mathbf{A}\) 列空间内的特征向量，记为 \(\bm{e}_{K+1},\bm{e}_{K+2},\dots,\bm{e}_M\)（方便起见，令\( \bm{e}_k \)的模长与\( \bm{a}_{\omega_k}\) 模长相同，都为\( \sqrt{M} \)）。由于协方差矩阵为共轭对称矩阵，其特征向量必然正交，因此有：
\[
...
Multiple Signal Classification (MUSIC) 方法正是利用了信号子空间与噪声子空间的正交性，实现对目标参数的高分辨率估计。给定一个导向矢量\( \bm{a}_{\omega} \)，如果其对应的参数值\( \omega \)恰好是某个目标的参数值\( \omega_k \)，那么该导向矢量必然位于信号子空间内，因此与噪声子空间正交。反之，如果\( \omega \)不是任何目标的参数值，那么导向矢量\( \bm{a}_{\omega} \)将位于噪声子空间内。

进一步地，可以通过将该导向矢量投影到噪声子空间来衡量其与噪声子空间的正交程度，即\( \|\mathbf{V}_{n}^{\mathrm{H}} \overline{\bm{a}}_{\omega}\|_2^2\)。如果该值接近于零，则说明\( \bm{a}_{\omega} \)与噪声子空间正交，进而推断出\( \omega \)可能对应某个目标的参数值，反之亦然。基于这一思路，MUSIC 方法定义了如下的谱函数：
\[
...
如\cref{fig_music_eg}所示，在强噪声环境下，CBF、Capon 与 MUSIC 三种方法的功率谱估计结果差异显著。可以观察到，MUSIC 方法的谱峰更加尖锐，旁瓣水平更低。同时，通过利用噪声子空间的正交特性，MUSIC 方法能够有效抑制噪声，使得其功率谱中的底噪水平明显低于 CBF 与 Capon，从而显著提升了目标的可检测性。
\begin{figure}[htb!]
    \centering
    \begin{tikzpicture}
        \begin{axis}[
...
与 CBF 和 Capon 方法不同，MUSIC 方法的推导过程中并未涉及滤波器权向量的设计，因此难以直接得到如 \cref{fig_compressed_capon} 所示的二维脉压结果。然而，MUSIC 的核心在于利用接收信号矩阵的低秩结构，通过对接收信号矩阵进行截断奇异值分解（Truncated SVD），也叫SVD降维，即仅保留前 \(K\) 个奇异值及其对应的奇异向量，对数据进行重构，从而实现信号成分的提取与噪声的抑制。
...
由此可见，右奇异向量矩阵 \(\mathbf{V}\) 正是协方差矩阵的特征向量矩阵，而奇异值的平方 \(\lambda_i^2\) 与协方差矩阵的特征值成正比。因此，前 \(K\) 个最大的奇异值对应于信号子空间，其余则对应于噪声子空间。据此，通过保留前 \(K\) 个奇异值及其对应的奇异向量，可近似提取信号子空间并抑制噪声。降噪后的数据矩阵为
\[
...
\begin{solution}
    ...
    可以对降噪后的数据矩阵先后采用匹配滤波和 Capon 算法进行二维脉压处理，从而获得更清晰的目标回波图像。需要注意的是，此时接收信号矩阵的协方差矩阵已不再满秩，因此在实现 Capon 算法时应引入对角加载技术。各算法处理结果的幅度图如\cref{fig_compressed_music}所示。由图可见，经过降噪处理后，目标信号得以更清晰地展现，旁瓣水平显著降低，从而有效提升了目标的分辨能力与检测性能。
...
\[
    p_{\omega} = \frac{1}{\overline{\bm{a}}_{\omega}^{\mathrm{H}}
        \mathbf{\Sigma}\,\overline{\bm{a}}_{\omega}},
\]
其中 Capon 方法对应 \(\mathbf{W} = \mathbf{\Sigma}_{\mathbf{X}}^{-1}\)，即对信号与噪声赋予与功率成反比的加权；而 MUSIC 方法对应 \(\mathbf{W} = \mathbf{V}_n \mathbf{V}_n^{\mathrm{H}}\)，是噪声子空间的投影矩阵，其权重仅取 \(0/1\)，本质上是一种非线性划分。正是这种权重结构的差异，使得 MUSIC 方法能够实现比 Capon方法 更高的分辨率。
...
给定接收信号矩阵\( \mathbf{X} = \mathbf{R} \mathbf{A}^{\mathrm{T}}  + \mathbf{N} \in \mathbb{C}^{L \times M} \)，其中导向矩阵\( \mathbf{A} \in \mathbb{C}^{M \times K} \)中的列向量的形式已知，为
\[
    \bm{a}_{\omega_k}
    = \begin{bmatrix}
        1 & e^{j2\pi\omega_k} & \cdots & e^{j2\pi(M-1)\omega_k}
...
\[
    p_i = \|\bm{s}_i\|, \quad i=1,2,\cdots,I,
\]
...
\[
    \begin{cases}
        ...
        \text{s.t.} \quad              & \|\mathbf{X} - \mathbf{S} \mathbf{D}^{\mathrm{T}}\|_{\mathrm{F}} \leq \epsilon, \\
                                       & \| \bm{s}_i \| = p_i, \quad i=1,2,\cdots,I,                                     \\
    \end{cases}
\]
...
\[
    \|\bm{p}\|_1 = \sum_{i=1}^I p_i = \bm{1}^{\mathrm{T}} \bm{p} = \sum_{i=1}^I \|\bm{s}_i\|.
\]
...
\[
    \begin{cases}
        ...
        \text{s.t.} \quad & \|\mathbf{X} - \mathbf{S} \mathbf{D}^{\mathrm{T}}\|_{\mathrm{F}} \leq \epsilon, \\
                          & \sum_{i=1}^I \|\bm{s}_i\| \leq \varepsilon,                                     \\
    \end{cases}
\]
其中\( \lambda > 0 \)为权衡参数，当\( \lambda \)较大时，更加注重系数向量的稀疏性，而当\( \lambda \)较小时，更加注重数据拟合的精度。

\begin{example}
    利用上述稀疏表示方法对\cref{fig_noise_esprit}所示的包含噪声的数据进行参数估计。
\end{example}
\begin{solution}
    ...
    从\cref{fig_sparse_result_1}可以看到，系数矩阵\( \mathbf{S} \)的列向量非常稀疏，只有两个分量显著非零，其余均接近于零。并且，非零的两个列向量就对应两个目标的回波信号。对应的功率谱如\cref{fig_sparse_result_2}所示，可以看到，稀疏表示方法成功地分辨出了两个目标参数，其谱峰位置与MUSIC方法的峰值位置基本一致，并且谱峰更加尖锐。
\end{solution}

需要注意的是，稀疏表示方法中权衡参数 \( \lambda \) 的选取对结果具有显著影响，通常需要通过多次实验进行调优。尽管稀疏表示方法在估计精度上往往优于传统方法，但其计算复杂度极高，运算耗时可能是 MUSIC 方法的数百甚至数千倍。为降低计算负担，可以先对接收数据矩阵进行奇异值分解（SVD），利用低秩近似来降维，从而在一定程度上牺牲精度以换取复杂度的下降。设接收数据矩阵为\( \mathbf{X} \in \mathbb{C}^{L \times M} \)，其SVD分解为
\[
    \mathbf{X} = \mathbf{U} \mathbf{\Lambda} \mathbf{V}^{\mathrm{H}}
    = \begin{bmatrix}
        \mathbf{U}_s & \mathbf{U}_n
...
\section{多域联合估计}
随着雷达探测技术的不断发展，现代雷达系统往往能够在多个维度上同时采集数据，例如方位、俯仰、多普勒等参数。这些多维信息天然适合组织为高阶张量形式。若仅截取其中某一维度的切片并采用传统参数估计方法进行处理，则不可避免地忽略其余维度所蕴含的结构特征，从而导致估计性能下降。为克服这一局限，近年来逐渐兴起了多域联合估计的方法，其核心思想是充分融合不同维度的信息，以实现更高精度和更强稳健性的目标参数估计。
...
\[
    \mathbf{Z} = \mathbf{S}^{\mathrm{H}} \mathbf{X} \mathbf{H},
\]
其中\( \mathbf{S}  \)为匹配滤波矩阵，\( \mathbf{H} \)为``波束形成''矩阵。这一形式自然引出一个问题：是否存在 CBF 的推广，使其能够同时作用于接收信号张量 \( \mathcal{X} \) 的多个维度，实现所谓的``多维脉冲压缩''，并使目标在结果中表现为清晰的峰值。
...
不难发现，就形式上而言，张量 \( \mathcal{Z} \) 实际上是二维脉压矩阵 \( \mathbf{Z} \) 的升维扩展。目标在该三维张量中依然表现为一个峰值，其三维坐标分别对应目标的距离、参数一与参数二。以多普勒线性阵列雷达系统为例，通过该方法即可在同一框架下实现对目标距离、方位角和速度的联合检测。下面，让我们对张量\( \mathcal{Z} \)进行进一步的分析，以证明其确实能够实现多维脉冲压缩的功能。
...
而矩阵 \( \tilde{\mathbf{A}} =  \mathbf{V}^{\mathrm{T}} \mathbf{A} \in \mathbb{C}^{P \times K} \) 对应第一个参数维的波束形成操作。记其列向量为
\[
...
\[
    ...
    \quad k=1,2,\dots,K,
\]
其中 \( \nu_k \) 为目标参数一的取值。类似地，矩阵 \(  \tilde{\mathbf{B}} = \mathbf{U}^{\mathrm{T}} \mathbf{B} \in \mathbb{C}^{Q \times K} \) 对应第二个参数维的波束形成操作。记其列向量为
\[
...
尽管无法直接得到该优化问题的解析解，但可以通过交替迭代的方法进行求解。此外，利用拉格朗日乘子法，可以构建如下的拉格朗日函数：
\[
...
\]
如将\( \mathbf{\Sigma}_{23} \)转换为一个4阶张量\( \mathcal{S} \in \mathbb{C}^{M \times N \times M \times N} \)，则滤波器的输出功率可以进一步表示为
\[
...
\]
其中信号部分可以看作是 \( K \) 个秩一张量的叠加。因此，作为SVD分解的推广，张量CP分解方法同样可以直接对接收数据张量进行处理，从而实现对目标参数的联合估计。对应的优化模型为
\[
...
\]
其中，\( \mathbf{E}_k \in \mathbb{R}^{K \times K} \)为一个稀疏矩阵，仅在第\( k \)列的第\( k \)行处取值为一，其余位置均为零。由此可见，矩阵 \( (\mathcal{I}_K)_{23} \) 实际上是一个选择矩阵，其作用是从矩阵 \( \mathbf{B} \otimes \mathbf{A} \) 的 \( K^2 \) 列中选择出其中的 \( K \) 列。注意到
\[
    (\mathbf{B} \otimes \mathbf{A}) (\mathcal{I}_K)_{23} = \sum_{k=1}^K (\mathbf{B} \otimes \bm{a}_k) \mathbf{E}_k \in \mathbb{C}^{M N \times K},
\]
其中
\[
    (\mathbf{B} \otimes \bm{a}_k) \mathbf{E}_k =\begin{bmatrix}
        0 & \cdots & \bm{b}_k \otimes \bm{a}_k & 0 & \cdots & 0
    \end{bmatrix} \in \mathbb{C}^{M N \times K},
\]
因此有
\[
    (\mathcal{I}_K \times_2 \mathbf{A} \times_3 \mathbf{B})_{23} = \begin{bmatrix}
        \bm{b}_1 \otimes \bm{a}_1 & \bm{b}_2 \otimes \bm{a}_2 & \cdots & \bm{b}_K \otimes \bm{a}_K
    \end{bmatrix} \in \mathbb{C}^{M N \times K}.
\]
上述操作也被记作 Khatri-Rao 积（Khatri-Rao Product）：
\[
...
此外，不难验证\( (\mathcal{I}_K \times_2 \mathbf{A} \times_3 \mathbf{B})_1 \in \mathbb{C}^{K \times M N} \)与\( \mathbf{B} \ast  \mathbf{A} = (\mathbf{B} \otimes \mathbf{A}) (\mathcal{I}_K)_{23} \in \mathbb{C}^{M N \times K} \)之间是转置关系，即
\[
    (\mathcal{I}_K \times_2 \mathbf{A} \times_3 \mathbf{B})_{1} = \begin{bmatrix}
        \bm{b}_1 \otimes \bm{a}_1 & \bm{b}_2 \otimes \bm{a}_2 & \cdots & \bm{b}_K \otimes \bm{a}_K
    \end{bmatrix}^{\mathrm{T}} \in \mathbb{C}^{K \times M N}.
\]
因此，矩阵 \( \mathbf{R} \) 对应的子优化问题可写成
\[
...
\]
类似地，可以分别对矩阵 \( \mathbf{A} \) 与 \( \mathbf{B} \) 进行优化，最终的更新公式如下：
\[
    \begin{cases}
        \mathbf{R} = \mathbf{X}_{1} (\mathbf{B} \ast  \mathbf{A}) \left( (\mathbf{B} \ast  \mathbf{A})^{\mathrm{T}} (\mathbf{B} \ast  \mathbf{A}) \right)^{-1}, \\
