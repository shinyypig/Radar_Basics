...
\]
类似地，可以分别对矩阵 \( \mathbf{A} \) 与 \( \mathbf{B} \) 进行优化，得到如下的闭式解：
\[
    \begin{cases}
    ...
    \end{cases}
\]
其中，\( \mathbf{X}_{2} \in \mathbb{C}^{M \times L N} \)与\( \mathbf{X}_{3} \in \mathbb{C}^{N \times L M} \)分别为张量\( \mathcal{X} \)的模-2与模-3展开矩阵。交替迭代上述三个子问题，直至收敛为止，即可得到最终的估计结果。
