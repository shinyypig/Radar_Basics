...
\]
其中 Capon 方法对应 \(\mathbf{\Sigma} = \mathbf{\Sigma}_{\mathbf{X}}^{-1}\)，即对信号与噪声赋予与功率成反比的加权；而 MUSIC 方法对应 \(\mathbf{\Sigma} = \mathbf{U}_n \mathbf{U}_n^{\mathrm{H}}\)，是噪声子空间的投影矩阵，其权重仅取 \(0/1\)，本质上是一种非线性划分。正是这种权重结构的差异，使得 MUSIC 能够实现比 Capon 更高的分辨率。
