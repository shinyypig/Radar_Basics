...
\[
    ...
    \end{split}
\]
可以发现该权重的取值仅和\( M \)以及参数差值\( \omega_k - \omega \)有关。如\cref{fig_weight}所示，随着\( M \)的增大，权重函数的主瓣变窄，旁瓣降低，对应的分辨率和也随之提升。
...
\]
直观地，我们希望输出 \(\bm{y}_{\omega}\) 尽可能只包含参数值为 \(\omega\) 的目标信号，同时抑制其他信号与噪声干扰。注意到目标信号对应的导向向量为 \(\bm{a}_{\omega}\)，因此目标分量在输出中的占比与 \(|\bm{a}_{\omega}^{\mathrm{T}} \bm{h}_{\omega}|\) 成正比。由此可构造如下优化问题：
\[
