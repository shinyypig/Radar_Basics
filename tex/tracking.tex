...
\]
注意到，随着观测次数 \(T\) 的增加，观测矩阵 \(\mathbf{X}_T\) 的规模不断增大，从而导致计算复杂度迅速提升。另一方面，目标的运动状态也可能随时间发生变化，因此没有必要始终依赖全部历史观测数据来估计观测矩阵 \(\mathbf{H}_T\)。基于此，可以考虑采用迭代方式对观测矩阵进行逐步更新。
...
\]
其中，\(\mathbf{H}_{T}\bm{x}_{T+1}-\bm{z}_{T+1}\) 即为利用当前估计矩阵对最新观测进行预测时所产生的误差。记\( \bm{e}_{T+1} = \bm{z}_{T+1} - \mathbf{H}_{T}\bm{x}_{T+1}\)，则可采用梯度下降法更新观测矩阵。对应的更新公式为
\[
    \mathbf{H}_{T+1} = \mathbf{H}_{T} + \mu\, \bm{e}_{T+1}\bm{x}_{T+1}^{\mathrm{T}},
\]
其中 \(\mu > 0\) 为步长参数。这种基于随机梯度下降（Stochastic Gradient Descent, SGD）更新的方法被称为最小均方（Least Mean Square, LMS）算法。可以发现其核心思想在于：每次引入新观测时，先利用当前观测矩阵进行预测并计算预测误差，随后结合该误差与当前观测对观测矩阵进行修正，从而实现动态更新。
