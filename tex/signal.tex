...
在第一章中我们提到，典型的雷达系统通常采用同一副天线进行信号的发射与接收。雷达通过发射信号，并测量接收回波与发射信号之间的时延来确定目标的距离。严格而言，这种工作方式主要适用于脉冲体制雷达，其发射信号为离散的脉冲序列。与之不同，连续波体制雷达持续向外辐射电磁波，并利用多普勒效应测量目标的速度，通过频率差测量目标的距离。鉴于实际应用中脉冲体制雷达更为常见，且连续波雷达的信号处理可在一定条件下转化为脉冲体制的处理形式，因此，本书将主要关注脉冲体制雷达的信号模型。
...
早期雷达结构简单，对应的低频包络表达式为\( p(t) = 1 \)，即发射信号可以看作是一个截断的复指数信号，如\cref{fig_radar_signal_1_1}所示。是否存在回波信号也通过简单的阈值来判断：设定一个固定的阈值，当接收到的信号强度超过该阈值时，认为检测到了目标回波。如\cref{fig_radar_signal_1_2}所示，当两个目标对应的回波信号之间的时延差小于脉冲宽度时，系统可能会将其误判为同一目标的回波。
...
所以，早期雷达的距离分辨率取决于发射脉冲的宽度，两者之间的关系可以用以下公式表示：
\[
    t_1 = \frac{2R_1}{c}, \qquad t_2 = \frac{2R_2}{c}.
\]
要在接收端分辨出这两个回波脉冲，它们在时间上的间隔必须不小于发射脉冲宽度 \(T\)，即
\[
    |t_1 - t_2| \ge T.
\]
代入上式可得
\[
    \left| \frac{2R_1}{c} - \frac{2R_2}{c} \right|
    = \frac{2}{c} |R_1 - R_2| \ge T,
\]
从而得到
\[
    |R_1 - R_2| \ge \frac{cT}{2}.
\]
因此，相邻两个目标之间能够被分辨开的最小距离（即距离分辨率）为
\[
    \Delta R = \frac{cT}{2},
\]
其中，\( c \)为光速，\( T \)为脉冲宽度，\( \Delta R \)为距离分辨率，是指能够分辨的相邻目标的最小距离，\( \Delta R \)的数值越小，代表分辨的能力越强，也就是距离分辨率越高。可以发现，脉冲宽度越小，距离分辨率越高。但是，由于发射功率一定，脉冲宽度变小的同时，发射信号的能量也会相应减少，这可能导致信号的信噪比降低，发射能到达的距离就会变近，从而影响目标的检测性能。换句话说，距离分辨率与目标检测性能之间存在一定的矛盾。

为兼顾距离分辨率与检测性能，现代雷达系统通常在低频包络 $p(t)$ 中引入时间调制，使其具有一定带宽。这样不仅能在保持发射信号能量的前提下提高距离分辨率，还能提升抗干扰能力与目标检测性能。常见的时间调制方式包括线性调频（Linear Frequency Modulation, LFM）、非线性调频（Nonlinear Frequency Modulation, NLFM）以及相位编码（Phase Coding）等，其中线性调频因其实现简单、性能稳定而被广泛采用。
...
\end{enumerate}
这些非线性调频形式能够在特定应用中优化脉冲压缩旁瓣特性，提升抗干扰能力和目标检测性能。但其实现复杂度相对较高，对硬件要求也更为严格。
...
除了频率调制，相位编码也是一种重要的信号调制手段，通过离散相位跳变实现特定性能。相位编码是一种通过在发射信号中引入离散相位变化来实现调制的技术，其核心思想是在信号持续时间内按照预定的编码序列对载波相位进行跳变。常见的相位编码方式包括：
\begin{enumerate}
    \item 二相编码（Binary Phase Shift Keying, BPSK）：相位取 $\{0, \pi\}$ 两个值，对应发射信号幅度符号为 $\{+1, -1\}$，典型代表为 Barker 码（如\cref{fig_barker}所示）；
...
如\cref{fig_array}所示，设第 $k$ 个目标与阵列天线的连线与阵列法向之间的夹角为 $\theta_k$。则第 $1$ 个阵元接收到的回波信号，相较于第 $0$ 个阵元接收到的信号，其传播路径的长度少了 $d \sin\theta_k$ 的距离。依次类推，第 $m$ 个阵元接收到的信号，相较于第 $0$ 个阵元接收到的信号，其传播路径的长度少了 $m d \sin\theta_k$ 的距离。不妨以第0个阵元为参考，令其接收到的第\( k \)个目标的回波信号为
\[
...
\]
请注意到，对于矩形窗函数和低频包络信号，它们的时间变化比较缓慢，时间尺度远远大于额外的时延$\frac{m d \sin\theta_k}{c}$，所以额外时延对它们的形状和幅度影响很小，可以忽略不计。而载波信号是高频信号，相位对时延非常敏感，相位的变化和载波频率乘以时延呈线性关系，所以带来的相位偏移不能忽略，必须保留。于是，上式可近似为
\[
    \begin{split}
...
在实际雷达系统中，连续的接收信号经过采样电路采样，再经模数转换，可以得到一系列的离散采样点。设每个阵元的采样点数为 \(L\)，则第 \(0\) 个阵元接收到的第 \(k\) 个目标的离散回波信号可表示为列向量
\[
    \bm{r}_k =
    \begin{bmatrix}
...
将\(M\) 个阵元接收到的所有第\(k\)个目标的回波信号按列排列，可以发现第\( k \)个目标对应的接收信号矩阵是一个秩一矩阵：
\[
    \begin{aligned}
        \mathbf{X}_k
         & = \begin{bmatrix}
...
\[
    ...
    \end{bmatrix}^{\mathrm{T}}
\]
称为\textbf{导向矢量}（Steering Vector），其元素之间的相位关系蕴涵了目标的方位信息。如图\ref{fig_array_angle}所示，当目标位于阵列的正前方时，所有阵元接收到的信号相位相同；当目标偏离阵列的正前方时，不同阵元接收到的信号之间会存在相位差。
...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \caption{一维线性阵列接收信号示例（仅绘制了实部） (a) 目标角度为 $0^\circ$ (b) 目标角度为正 (c) 目标角度为负}
...
\]
其中 \(\lambda_c = \tfrac{c}{f_c}\) 为载波波长。另一方面，\(\bm{a}_{\theta_k}\) 可以理解为以单位采样率（每阵元采样一次）对该复指数信号进行采样得到的向量。根据奈奎斯特采样定理，为避免频率混叠，必须满足
\[
    |f_k| \leq \frac{1}{2}.
\]
因此，为防止出现空间混叠，阵元间距应满足
\[
    d \leq \frac{\lambda_c}{2}.
\]
...
\]
其中\( \mathbf{R} \)被称为目标信号矩阵，包含了目标的距离信息，\( \mathbf{A} \)被称为导向矩阵（Steering Matrix）或阵列流形矩阵（Array Manifold Matrix）包含了目标的方位信息。
...
在二维阵列结构中，由于各阵元相对于参考阵元的位置不同，其接收回波会相对于原点阵元引入额外的路径差，从而造成附加时延。根据\cref{fig_2d_array_eg}，又因为信号会被目标同时反射，在发射信号到反射过程不会产生相位差，故在整个过程中，第\( (m,n) \)个阵元仅仅在信号被目标反射到再次接收的过程中，相对\( (0,0) \)阵元接收到的信号减少一个传播路径差，为
\[
...
\[
    ...
    \end{split}
\]
同样地，令\( \bm{r}_k \) 为第\( (0,0) \)个阵元接收到第\(k\)个目标反射的信号对应的离散向量，则第\( (m,n) \)个阵元接收到的第k个目标的反射信号对应的离散信号向量可以表示为
\[
...
因此，在理想条件下，二维均匀线性阵列的接收信号张量可以近似表示为一组秩一张量的叠加：
\[
    \mathcal{X}
...
\[
    \begin{split}
        \mathcal{X} = \mathcal{I}_3 \times_1 \mathbf{R} \times_2 \mathbf{A} \times_3 \mathbf{B}.
    \end{split},
\]
...
当二维阵列的第二个维度\( N=1 \)时，阵列退化为一维阵列，接收张量可以表示为
\[
...
\section{多普勒接收信号模型}
天线阵列的空间维度的展开赋予雷达``方向分辨力''，从而测量目标的方位；而时间维度的展开则提供了``运动分辨力''，用以估计目标的速度。相较于需要多阵元同步采样的空间维展开，时间维展开的实现更为直接，只需进行脉冲的重复发射与接收。

设雷达系统只有一个天线，且每隔 \(T_r\) 秒发射一个脉冲，即脉冲重复周期（Pulse Repetition Interval, PRI）为 \(T_r\)，并持续发射 \(P\) 个脉冲。与此同时，设第\( k \)个目标的径向速度为\( v_k \)，且第 \( p \) 个脉冲发射的时刻为\( p T_r \)。如\cref{fig_v_k}所示，此时目标到雷达的距离近似为\( R_k -  p T_r v_k\)，其中\( R_k \)为初始时刻目标到雷达的距离。
...
\[
    \bm{r}_k^{(p)}(t) = A_k s\left(t - \frac{2 (R_k - p T_r v_k)}{c}\right).
\]
将发射信号的基本模型代入到上式中，有
\[
    \begin{split}
        \bm{r}_k^{(p)}(t) & = A_k \text{rect}\left(\frac{t - \frac{2 (R_k - p T_r v_k)}{c}}{T}\right) p\left(t - \frac{2 (R_k - p T_r v_k)}{c}\right) e^{j 2 \pi f_c \left(t - \frac{2 (R_k - p T_r v_k)}{c}\right)}   \\
    ...
    \end{split}
\]
其中，\( r_k(t) \)为第0次发射脉冲对应的接收信号。对比第 0 次，第\( p \)次脉冲的接收信号多了一个相位因子，该因子与\( p \)相关，且包含目标径向速度信息。

同样地，令\( \bm{r}_k \) 为第0次发射脉冲对应的接收信号向量，则第\( p \)次脉冲对应的接收信号向量可以表示为
\[
    \bm{r}_k^{(p)} = \bm{r}_k e^{j 2 \pi f_c \frac{2 p T_r v_k}{c}}.
\]
将\( P \)次发射脉冲对应的接收信号向量组合成一个矩阵，可以表示为
\[
    \begin{split}
...
\[
    ...
    \end{bmatrix}^{\mathrm{T}},
\]
其同样也是一个导向矢量，其中包含了目标的径向速度信息。注意到，矩阵 \( \mathbf{X}_k \in \mathbb{C}^{L \times P} \) 的两个维度都与时间相关：其中，维度 \(L\) 对应雷达系统对回波信号的采样时刻，而维度 \(P\) 则对应脉冲的重复发射次数。由于前者的采样间隔远小于后者，通常将 \(L\) 所在的维度称为``快时间''维，而将 \(P\) 所在的维度称为``慢时间''维。
...
\[
    \mathbf{R} = \begin{bmatrix}
        \bm{r}_1 & \bm{r}_2 & \cdots & \bm{r}_K
    \end{bmatrix}\text{、} \quad \mathbf{P} = \begin{bmatrix}
        \bm{p}_{v_1} & \bm{p}_{v_2} & \cdots & \bm{p}_{v_K}
    \end{bmatrix}
\]
分别是由目标接收信号和导向矢量构成的矩阵。

类似于一维线性阵列中的空间混叠现象，在多普勒体制的雷达中同样可能出现速度模糊。由于\(\bm{p}_{v_K}\) 也是一段复指数序列，其对应的``多普勒频率''为
\[
    f_k = f_c \frac{2 T_r v_k}{c}.
\]
由于本章采用的是脉冲体制雷达，采样得到的信号均为时域离散信号，根据数字信号的奈奎斯特采样定理，以及采用 Hz为单位，数字采样频率为1，为避免混叠必须满足
\[
    |f_k| \leq \frac{1}{2}.
\]
因此，目标径向速度的可测范围为
\[
    ...
    = \frac{\lambda_c f_r}{4},
\]
其中 \(\lambda_c = c/f_c\) 为载波波长，\(f_r = 1/T_r\) 为脉冲重复频率（Pulse Repetition Frequency, PRF）。除此之外，多普勒体制的雷达还可能出现距离模糊现象。由于脉冲重复周期为 \(T_r\)，当目标距离 \(R_k\) 大于 \(\tfrac{c T_r}{2}\) 时，其回波信号至少要到下一次脉冲发射之后才能返回接收机，从而导致目标距离无法被正确估计。由此可得，多普勒雷达的无模糊测距范围为
\[
...
在前面的推导中，我们分别从空间维度（阵列接收）和时间维度（脉冲重复）出发，建立了目标回波的接收信号模型。实际上，现代雷达系统往往需要在空间与时间两个维度上同时处理数据：一方面，阵列天线提供了空域采样能力，可以用于角度估计与波束形成；另一方面，脉冲重复采样提供了时域上的信息，可用于目标检测、速度估计以及杂波抑制。将这两个维度结合起来，可以得到所谓的``空时联合接收信号模型''（space-time signal model），为后续的空时自适应处理（Space-Time Adaptive Processing, STAP）奠定基础。本节将建立这一联合模型，并为后续算法的推导提供统一的数学表示。

设有一套多普勒二维阵列雷达系统，阵列规模为 \(M \times N\)，在两个正交方向上的阵元间距分别为 \(d_x\) 和 \(d_y\)。系统的脉冲重复周期为 \(T_r\)，一次观测中连续发射 \(P\) 个脉冲。考虑场景中存在 \(K\) 个目标，第 \(k\) 个目标的径向距离为 \(R_k\)，径向速度为 \(v_k\)，其相对于阵列坐标系的俯仰角为 \(\theta_k\)，方位角为 \(\phi_k\)。则第 \((m,n)\) 个阵元接收到的第 \(p\) 个脉冲对应的回波信号可以表示为
\[
    \begin{split}
        r^{(m,n,p)}_k(t)
...
\[
    ...
    \nu_k = 2 T_r v_k.
\]
分别表示目标在两个空间维度上的相位偏移量以及在慢时间维度上的多普勒相位偏移量。令\( \bm{r}_k \in \mathbb{C}^{L} \) 为对 \(r_k(t)\) 进行离散采样所获得的向量，并构造如下导向矢量
\[
    \begin{split}
        \bm{a}_{\vartheta_k} & = \begin{bmatrix}
                                     1 & e^{j 2 \pi f_c \frac{\vartheta_k}{c}} & \cdots & e^{j 2 \pi f_c \frac{(M-1) \vartheta_k}{c}}
...
在前面的讨论中，我们主要关注了理想情况下的雷达接收信号模型。然而，在实际应用中，雷达系统不可避免地会受到各种噪声、杂波和多径效应的影响，这些因素会显著干扰目标信号的检测与估计。因此，建立包含这些干扰因素的更贴近实际的接收信号模型，对于设计鲁棒的雷达信号处理算法至关重要。

本书中的噪声通常指热噪声，由接收机前端、信道特性和环境因素共同作用产生，一般可建模为加性高斯白噪声（Additive White Gaussian Noise, AWGN）。这里我们以一维均匀线性阵列为例，设一共有\( M \)个阵元，那么每个阵元在采集信号的过程中都不可避免地会被噪声干扰。令采样点数为\( L \)，则接收数据为一个\( L \times M \)的矩阵，可以表示为
\[
...
\]
这一性质在实际应用中是非常重要的，因为它意味着在信号处理过程中可以将噪声视为与信号相互独立的干扰项，从而简化了许多算法的设计与分析。但是由于硬件滤波、带宽限制等因素，噪声也可能呈现出非高斯特性，对应的协方差矩阵有可能不再是对角矩阵，这类噪声被称为色噪声（Colored Noise）。在这种情况下，噪声的统计特性会更加复杂，需要采用更为精细的模型来描述其影响，本书不做深入探讨。

除了噪声以外，杂波也是影响雷达系统探测与成像性能的重要因素。杂波通常是指来自环境的非目标回波，例如地面或海面、雨滴、鸟类等反射引起的回波，这些杂波信号会与目标信号混叠在一起，增加了目标检测和定位的难度。

以地杂波为例，由于地面通常是相对固定的散射体，比如建筑、山体、树林等，且在较小的范围内这类目标数量很多，因此产生的地杂波可以看作是大量幅度微弱的回波信号的叠加。根据中心极限定理，这些微弱回波信号的叠加可以近似看作一个高斯随机过程，因此地杂波通常也可以建模为加性高斯噪声。然而，与热噪声不同的是，地杂波往往具有较强的空间相关性和时间相关性，这使得其协方差矩阵不再是一个对角矩阵，反映了杂波在不同阵元和不同时间点之间的相关性。不同的地形特征和环境条件也会对杂波的特性产生影响，对应的杂波模型也完全有可能不再服从高斯分布，需要根据具体应用场景进行调整。

与地杂波相比，海杂波通常表现出更加复杂的特性。海面是一个动态变化的散射体，其反射特性不仅受到风速的影响，还与海洋温度、流场等多种环境因素密切相关。因此，在对海杂波进行建模时，往往需要引入更多环境变量，并采用更复杂的统计模型来刻画其特性。在海面较为平静、风速较小时，海面上主要由大量微小波浪构成，此时的杂波可以近似看作众多微弱回波的叠加，类似于地杂波，能够建模为加性高斯噪声。而当风速较大、海况较为复杂时，海杂波的幅度分布则会呈现明显的``尖峰''与``长尾''特征。这是因为此时杂波不再是单一高斯过程，而是由多个具有不同功率水平的高斯过程复合而成。换言之，尽管局部小波浪的回波叠加可以近似为高斯分布，但在大风浪作用下，各种尺度的海浪同时存在，最终的回波分布表现为多个高斯过程的混合。该类复合过程对应的幅值分布即为 K 分布（K-distribution），目前已被广泛应用于海杂波的建模与分析中。除此之外，Weibull 分布、Log-normal 分布等也常被用于描述海杂波的幅度特性。

除了噪声与杂波之外，多径效应也是雷达系统中普遍存在且不可忽视的干扰因素。所谓多径，是指电磁波在传播过程中不仅沿着目标的直达路径到达接收端，还会在地面、海面、建筑物或其他障碍物上发生反射、散射或绕射，从而形成多条传播路径。接收机最终接收到的信号是这些不同路径回波的叠加。
...
多径效应可能导致接收信号出现展宽，使脉冲波形被拉长甚至发生畸变，从而降低系统的时延分辨率。不同路径分量在接收端叠加时，可能产生相长或相消干涉：若多径分量之间具有固定的相位关系，则会形成稳定的干涉条纹；若相位随时间快速变化，则表现为信号强度的随机起伏，即常说的``衰落（Fading）''。此外，在雷达成像与检测中，多径分量还可能``伪目标''的形式出现。例如，海面或地面反射路径可能导致目标被``抬高''或``镜像化''，从而引发误判；在复杂环境下，甚至可能出现多个虚假目标与真实目标混叠的情况，增加目标检测与识别的难度。
