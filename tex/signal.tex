...
早期雷达结构简单，对应的低频包络表达式为\( p(t) = 1 \)，即发射信号是一个截断的复指数信号。判断是否有回波信号也是通过简单的阈值判断，通常设定一个固定的阈值，当接收到的信号强度超过该阈值时，认为检测到了目标回波。因此，当两个目标对应的回波信号之间的时延差小于脉冲宽度时，系统可能会将其误判为同一目标的回波。所以，早期雷达的距离分辨率取决于发射脉冲的宽度。为便于定量分析，设存在两个目标，它们到雷达的距离分别为\(R_1\) 和 \(R_2\)，则对应回波到达接收机的时间为
\[
...
\[
    \Delta R = \frac{cT}{2},
\]
其中，\( c \)为光速，\( \Delta R \)为距离分辨率，是指能够分辨的相邻目标的最小距离，\( \Delta R \)的数值越小，代表分辨的能力越强，也就是距离分辨率越高。可以发现，脉冲宽度越小，距离分辨率越高。但是，由于发射功率一定，脉冲宽度变小的同时，发射信号的能量也会相应减少，这可能导致信号的信噪比降低，发射能到达的距离就会变近，从而影响目标的检测性能。换句话说，距离分辨率与目标检测性能之间存在一定的矛盾。
...
\]
其中 \(\lambda_c = \tfrac{c}{f_c}\) 为载波波长。另一方面，\(\bm{a}_{\theta_k}\) 可以理解为以单位采样率（每阵元采样一次）对该复指数信号进行采样得到的向量。根据奈奎斯特采样定理，为避免频率混叠必须满足
\[
    |f_k| \leq \frac{1}{2}.
\]
...
\]
其中\( \mathbf{R} \)被称为目标信号矩阵包含了目标的距离信息，\( \mathbf{A} \)被称为导向矩阵（Steering Matrix）或阵列流形矩阵（Array Manifold Matrix）包含了目标的方位信息。
...
设雷达系统只有一个天线，且每隔 \(T_r\) 秒发射一个脉冲，即脉冲重复周期（Pulse Repetition Interval, PRI）为 \(T_r\)，并持续发射 \(P\) 个脉冲。与此同时，设第\( k \)个目标的径向速度为\( v_k \)，则第 \( p \) 个脉冲发射的时刻为\( p T_r \)。如\cref{fig_v_k}所示，此时目标到雷达的距离近似为\( R_k -  p T_r v_k\)，其中\( R_k \)为初始时刻目标到雷达的距离。
...
以地杂波为例，由于地面通常是相对固定的散射体，比如建筑、山体、树林等，且在较小的范围内这类目标数量很多，因此产生的地杂波可以看作是大量幅度微弱的回波信号的叠加。根据中心极限定理，这些微弱回波信号的叠加可以近似看作是一个高斯随机过程，因此地杂波通常也可以建模为加性高斯噪声。然而，与热噪声不同的是，地杂波往往具有较强的空间相关性和时间相关性，这使得其协方差矩阵不再是一个对角矩阵，反映了杂波在不同阵元和不同时间点之间的相关性。不同的地形特征和环境条件也会对杂波的特性产生影响，对应的杂波模型也完全有可能不再服从高斯分布，需要根据具体应用场景进行调整。
