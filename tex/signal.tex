...
\[
    \Delta R = \frac{cT}{2},
\]
其中，\( \Delta R \)为距离分辨率，\( T \)为脉冲宽度，\( c \)为光速。可以发现，脉冲宽度越小，距离分辨率越高。但是，脉冲宽度变小的同时，发射信号的能量也会相应减少，这可能导致信号的信噪比降低，从而影响目标的检测性能。换句话说，距离分辨率与目标检测性能之间存在一定的矛盾。
...
以一维均匀线性阵列（Uniform Linear Array, ULA）为例，设有 $M$ 个天线单元（阵元），且阵元间距为 $d$。在目标足够远的情况下，目标反射的电磁波可以近似看作平面波。
...
\]
其中\( \mathbf{R} \)被称为目标信号矩阵，包含了目标的距离信息，\( \mathbf{A} \)被称为导向矢量矩阵（Steering Matrix）或阵列流形矩阵（Array Manifold Matrix），包含了目标的方位信息。
