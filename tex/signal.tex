...
早期雷达结构简单，对应的低频包络表达式为\( p(t) = 1 \)，即发射信号是一个截断的复指数信号。是否存在回波信号也只通过简单的阈值判断：当接收到的信号强度超过该阈值时，便认为检测到了目标回波。因此，当两个目标对应的回波信号之间的时延差小于脉冲宽度时，系统可能会将其误判为同一目标的回波。设目标间的距离差为 \( \Delta R \)，则对应的时延差为 \( \frac{2 \Delta R}{c} \)，其中 \( c \) 为光速。为了能够区分这两个目标，必须满足
\[
    \frac{2 \Delta R}{c} > T,
\]
换而言之，早期雷达的距离分辨率取决于发射脉冲的宽度，两个目标之间的距离差必须满足
\[
    \Delta R > \frac{cT}{2}.
\]
可以看出，脉冲宽度越窄，距离分辨率越高。然而需要注意的是，在雷达系统发射功率保持不变的情况下，脉冲宽度减小意味着单个脉冲所携带的能量相应减少，这可能导致信噪比的下降，进而影响目标检测性能。换言之，距离分辨率的提高与检测性能之间存在一定的权衡关系。
...
为兼顾距离分辨率与检测性能，现代雷达系统通常在低频包络 $p(t)$ 中引入时间调制，使其具有一定带宽。这样不仅可以在不降低发射信号能量的情况下提高距离分辨率，还可以进一步提升抗干扰能力与目标检测性能。常见的时间调制方式包括线性调频（Linear Frequency Modulation, LFM）、非线性调频（Nonlinear Frequency Modulation, NLFM）以及相位编码（Phase Coding）等，其中线性调频因其实现简单、性能稳定而被广泛采用。
...
除此以外，相位编码也是一种常用的调制方式，其核心思想是在信号持续时间内按照预定的编码序列对载波相位进行跳变。常见的相位编码方式包括：
\begin{enumerate}
    \item 二相编码（Binary Phase Shift Keying, BPSK）：相位取 $\{0, \pi\}$ 两个值，典型代表为 Barker 码（如\cref{fig_barker}所示）；
...
以一维均匀线性阵列（Uniform Linear Array, ULA）为例，设有 $M$ 个天线单元（阵元），且阵元间距为 $d$。在目标足够远的情况下，目标反射的电磁波可以近似为平面波。
...
如\cref{fig_array}所示，设第 $k$ 个目标与阵列天线的连线与阵列法向之间的夹角为 $\theta_k$。由于信号通常由同一发射天线辐射，且该发射天线往往与接收阵列的距离较近，因此可以认为各阵元接收到的回波信号，仅在目标反射回阵列这一传播路径上存在差异。具体而言，第 0 个阵元接收到的回波信号，相较于第 1 个阵元，其传播路径缩短了 \( d \sin\theta_k \)；依此类推，第 \( m \) 个阵元相对于第 0 个阵元，其传播路径缩短了 \( m d \sin\theta_k \)。不妨以第0个阵元为参考，令其接收到的第\( k \)个目标的回波信号为
\[
...
\]
注意到，对于矩形窗函数和低频包络信号，额外的时延 $\frac{m d \sin\theta_k}{c}$ 对其影响可以忽略不计。而对于高频的载波信号，该时延会引入显著的相位变化。于是，上式可近似为
\[
    \begin{split}
...
注意到，\(\bm{a}_{\theta_k}\) 可以看作是一个以单位采样率在空间上进行采样（每阵元采样一次）获得的复指数序列，并且其对应的``空间频率''为
\[
...
\]
其中 \(\lambda = \tfrac{c}{f_c}\) 为载波波长。而根据奈奎斯特采样定理，为避免频率混叠，最高频率必须满足
\[
    |f_k| < \frac{1}{2}.
\]
因此，为防止出现角度上的模糊，阵元间距应满足
\[
    d \leq \frac{\lambda}{2}.
\]
...
类似于一维阵列，若仅考虑第\( k \)个目标，则二维阵列中的阵元接收到的信号，相较于位于原点处的参考阵元，会引入额外的时延。根据\cref{fig_2d_array_eg}，可以发现，第\( (m,n) \)个阵元接收到的信号少走的距离为
\[
    \Delta d^{(m,n)}_k = \bm{u}_k^{\mathrm{T}} \bm{l}^{(m,n)} = m d_x \sin\theta_k \cos\phi_k + n d_y \sin\theta_k \sin\phi_k.
\]
记\( \vartheta_k = d_x \sin\theta_k \cos\phi_k \)，\( \varphi_k = d_y \sin\theta_k \sin\phi_k \)，则第\( (m,n) \)个阵元接收到的第\( k \)个目标的信号可以表示为
\[
    \begin{split}
...
\[
    ...
    \end{bmatrix}^{\mathrm{T}}.
\]
注意到\( \bm{p}_{v_k} \)同样是一个导向矢量，其包含了目标的径向速度信息。注意到，矩阵 \( \mathbf{X}_k \in \mathbb{C}^{L \times P} \) 的两个维度都与时间相关：其中，维度 \(L\) 对应雷达系统对回波信号的采样时刻，而维度 \(P\) 则对应脉冲的重复发射次数。由于前者的采样间隔远小于后者，通常将 \(L\) 所在的维度称为“快时间”，而将 \(P\) 所在的维度称为“慢时间”。
...
\[
    ...
    = \frac{c^2}{8 f_c}.
\]
这表明，无模糊速度与无模糊距离之间存在乘积约束，其上限仅由载波频率 \(f_c\) 决定。换句话说，若希望同时增大可测速度与可测距离，就必须降低载波频率；然而工作频率的降低又会使雷达系统的体积增大、分辨率下降，从而产生新的折中问题。
...
以地杂波为例，由于地面通常是相对固定的散射体，比如建筑、山体、树林等，且在较小的范围内这些目标数量很多，因此产生的地杂波可以看作是大量的幅度微弱的回波信号的叠加。根据中心极限定理，这些微弱回波信号的叠加可以近似看作是一个高斯随机过程，因此地杂波通常也可以建模为加性高斯噪声。然而，与热噪声不同的是，地杂波往往具有较强的空间相关性和时间相关性，这使得其协方差矩阵不再是一个对角矩阵，反映了杂波在不同阵元和不同时间点之间的相关性。不同的地形特征和环境条件也会对杂波的特性产生影响，对应的杂波模型也完全有可能不再服从高斯分布，需要根据具体应用场景进行调整。

与地杂波相比，海杂波通常表现出更加复杂的特性。海面是一个动态变化的散射体，其反射特性不仅受到风速的影响，还与海洋温度、流场等多种环境因素密切相关。因此，在对海杂波进行建模时，往往需要引入更多环境变量，并采用更复杂的统计模型来刻画其特性。在海面较为平静、风速较小时，海面上主要由大量微小波浪构成，此时的杂波可以近似看作众多微弱回波的叠加，类似于地杂波，能够建模为加性高斯噪声。而当风速较大、海况较为复杂时，海杂波的幅度分布则会呈现明显的``尖峰''与``长尾''特征。这是因为海杂波不再是单一高斯过程，而是由多个具有不同功率水平的高斯过程复合而成。换言之，尽管局部小波浪的回波叠加可以近似为高斯分布，但在大风浪作用下，各种尺度的海浪同时存在，最终的回波分布表现为多个高斯过程的混合。该类复合过程对应的幅值分布即为 K 分布（K-distribution），目前已被广泛应用于海杂波的建模与分析中。除此之外，Weibull 分布、Log-normal 分布等也常被用于描述海杂波的幅度特性。

除了噪声与杂波之外，多径效应也是雷达系统中普遍存在且不可忽视的干扰因素。所谓多径，是指在传播过程中，电磁波不仅沿着目标的直达路径到达接收端，还会在地面、海面、建筑物或其他障碍物上发生反射、散射或绕射，从而形成多条传播路径。此时，接收机最终接收到的信号是这些不同路径回波的叠加。
...
多径效应可能导致接收信号发生展宽，使回波脉冲的波形被拉伸甚至产生畸变，从而降低系统的时延分辨率。不同路径分量在接收端叠加时，可能产生相长或相消干涉：若多径分量之间具有固定的相位关系，则会形成稳定的干涉条纹；若相位随时间快速变化，则表现为信号强度的随机起伏，即常说的``衰落''（Fading）。此外，在雷达成像与检测中，多径分量还可能以``伪目标''的形式出现。例如，海面或地面反射路径可能导致目标被``抬高''或``镜像化''，从而引发误判；在复杂环境下，甚至可能出现多个虚假目标与真实目标混叠，增加目标检测与识别的难度。
