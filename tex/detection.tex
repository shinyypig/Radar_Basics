...
为克服距离分辨率与检测性能之间的固有矛盾，现代雷达系统中中引入了脉冲压缩（Pulse Compression）技术。该方法通过在发射端发送宽脉冲、在接收端对回波进行匹配滤波，实现了``能量积累''与``时间压缩''的统一。由此，雷达既能在保持高能量积累以提高检测概率的同时，又获得等效的窄脉冲，从而显著提升距离分辨率。
...
由\cref{apx.conv-corr-mat}可知，该形式实质上对应一次相关运算，即将原始回波信号与参考信号进行相关处理。进一步地，相关运算可以等价地转化为卷积运算，而卷积又可视为一个有限冲激响应（Finite Impulse Response, FIR）滤波器，其滤波器系数正是参考信号的共轭翻转序列。因此，该处理过程通常被称为\emph{匹配滤波}。为方便起见，我们直接称\( \bm{s} \)为匹配滤波器的滤波器系数。
...
\begin{solution}
    ...
    \]
    其中\( s_k \)为参考信号的第\( k \)个采样点， \( n \) 为噪声项。注意到，对于线性调频信号而言，归一化后的参考向量\( \bm{s} \in \mathbb{C}^{W \times 1} \)中的元素\( s_k \)的模值均为\( \frac{1}{\sqrt{W}} \)。下面给出归一化后每个样点模值为 \( \frac{1}{\sqrt{W}} \) 的简洁推导：
    \[
    ...
    不妨令向量\( \bm{x}_i = A \bm{s} + \bm{n}_i \)对应了目标的回波信号，其中\(\bm{n}_i\) 为噪声向量。那么脉压后，目标对应的尖峰值取值的均值为
    \[
...
\[
    ...
    \end{cases}.
\]
从中可以看出，最优的滤波器系数 \( \bm{w} \) 并不等于参考信号 \( \bm{s} \)。但注意到，当我们假设信号中只存在高斯白噪声时，数据的协方差矩阵近似为一个对角矩阵\( \frac{1}{L-W+1} \mathbf{X} \mathbf{X}^{\mathrm{H}} \approx \sigma^2_n \mathbf{I} \)，其中\( \mathbf{I} \)为单位矩阵。此时，最优的滤波器系数 \( \bm{w} \) 近似为
\[
...
此外需要注意，脉冲压缩后的输出脉冲实际上对应于参考向量的自相关，因此其波形形状与参考向量（低频包络）密切相关。为便于理论推导，下文采用连续时间形式进行分析；而实际系统中的离散自相关结果，可视为对连续自相关函数的采样近似。注意到，发射信号的低频包络由两部分组成
\[
...
这表明，脉冲压缩输出的脉冲形状近似为\( p(t) \)自相关函数与三角窗的乘积。而一个信号的自相关函数宽度取决于其带宽：带宽越大，自相关函数越窄，反之亦然。若假设 \(p(t)\) 的带宽为 \(B\)，则其自相关函数宽度约为 \(1/B\)。另一方面，若两个目标间的距离差为 \(\Delta R\)，则其回波经脉冲压缩后的时间间隔为 \(\frac{2\Delta R}{c}\)。因此，只有当\(\frac{2\Delta R}{c} > \frac{1}{B}\)时，两个脉冲才能被有效区分。由此可见，脉冲压缩技术能够将雷达的距离分辨率提升至
\[
    \Delta R = \frac{c}{2B}.
\]
...
在\cref{eg_pc}中，我们已经证明，脉冲压缩所带来的信噪比提升倍数等于参考向量的长度 \(W\)。根据香农采样定理，对于带宽为\( B \)的复信号，采样率 \(f_s\) 至少应满足 \(f_s \geq B\)。因此，如果发射脉冲的宽度 \(T\) ，则对应的参考向量的采样点数至少为
\[
    W = f_s T \geq BT.
\]
...
在完成脉冲压缩与脉冲积累后，雷达系统需要对处理后的信号进行目标检测，以区分目标回波、噪声或杂波。目标检测的核心是信号存在性判定，通常建模为二元假设检验问题，即在给定观测数据的情况下，判断其是来自仅含噪声（记作\( H_0 \)）还是包含目标（记作 \( H_1 \)）。基于统计决策理论，可以设计各种检测算法以优化检测性能。
...
\]
其中\( p_0 \)和\( p_1 \)分别是目标和噪声的概率密度函数。设\( \Phi_{0} \) 和\( \Phi_{1} \)分别为其对应的累积分布函数，即
\[
...
\]
其中 \(X_i\) 为待检测样本（Cell Under Test, CUT）。最常见的 CFAR 算法是基于滑动窗口的单元平均恒虚警率检测（Cell Averaging CFAR, CA-CFAR）。该方法通过在CUT周围选取训练单元并估计背景噪声功率，从而自适应调整检测门限。
...
\]
当 \(X_i > \hat{\gamma}_i\) 时判定存在目标，否则判定为噪声。此外，之所以要设置保护单元，是因为脉压后的脉冲仍有一定的宽度，且往往存在旁瓣。如果不设置保护单元，目标主瓣或旁瓣的能量就会被误算进训练单元，导致背景噪声估计被抬高，阈值过高，从而削弱检测灵敏度甚至漏检目标。
